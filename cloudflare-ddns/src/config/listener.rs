...
pub async fn load() -> Result<(DDNSContext, UpdatersManager, ConfigStorage)> {
    ...
    macro_rules! exists_or_include {
        ($($name: literal @ $include: expr),*) => {
            tokio::try_join!($(async {
                if !tokio::fs::try_exists(concat!("./config/", $name, ".toml")).await? {
                    tokio::fs::write(concat!("./config/", $name, ".toml"), $include).await?;
                }
                Ok::<_, io::Error>(())
            }),*)
        };
    ...
    exists_or_include!(
        "api" @ include_str!("../../includes/api.toml"),
        "http" @ include_str!("../../includes/http.toml"),
        "misc" @ include_str!("../../includes/misc.toml"),
        "sources" @ include_str!(concat!(env!("OUT_DIR"), "/sources.toml"))
    )?;
